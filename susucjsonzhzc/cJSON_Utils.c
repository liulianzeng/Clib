...
{ 
   ...
   void *new = (void *)susu_cjson_malloc(len); 
   if (new == NULL) 
      return NULL; 
...
char *cJSONUtils_FindPointerFromObjectTo(cJSON *object,cJSON *target)
{
	...
	for (obj=object->child;obj;obj=obj->next,c++)
	{
		...
		if (found)
		{
			if (type==cJSON_Array)
			{
				char *ret=(char*)susu_cjson_malloc(strlen(found)+23);
				sprintf(ret,"/%d%s",c,found);
				susu_cjson_free(found);
				return ret;
			}
			else if (type==cJSON_Object)
			{
				char *ret=(char*)susu_cjson_malloc(strlen(found)+cJSONUtils_PointerEncodedstrlen(obj->string)+2);
...
static int cJSONUtils_ApplyPatch(cJSON *object,cJSON *patch)
{
	...
	if (!parent) {susu_cjson_free(parentptr); return 9;}	/* Couldn't find object to add to. */
...
static void cJSONUtils_GeneratePatch(cJSON *patches,const char *op,const char *path,const char *suffix,cJSON *val)
{
	...
	if (suffix)
	{
		char *newpath=(char*)susu_cjson_malloc(strlen(path)+cJSONUtils_PointerEncodedstrlen(suffix)+2);
...
static void cJSONUtils_CompareToPatch(cJSON *patches,const char *path,cJSON *from,cJSON *to)
{
	...
	case cJSON_Array:
	{
		int c;char *newpath=(char*)susu_cjson_malloc(strlen(path)+23);	/* Allow space for 64bit int. */
	...
	case cJSON_Object:
	{
		...
		while (a || b)
		{
			...
			if (!diff)
			{
				char *newpath=(char*)susu_cjson_malloc(strlen(path)+cJSONUtils_PointerEncodedstrlen(a->string)+2);
